...
function save_handler() {
	...
	if(timer_started) {
		...
		var response = id + " " + image.boxes.length + " " + time + " ";

		for(var i = 0; i < image.boxes.length; i++) {
		...
		xmlhttp.open("GET", "hpu3/save/" + response, true)
		xmlhttp.send()
...
function x_handler() {
	...
	if("active_box" in image) {
		...
		if(image.active_box > -1) {
			...
			if(image.boxes[index].h < 400) {

				image.boxes[index].w += 1.0 * image.scale;
				image.boxes[index].h += 1.25 * image.scale;
		...
		else if(image.active_box == -2 && image.temp_box.h < 400) {

			var cx = image.temp_box.x + 0.5 * image.temp_box.w;
			var cy = image.temp_box.y + 0.5 * image.temp_box.h;

			image.temp_box.w += 1.0 * image.scale;
			image.temp_box.h += 1.25 * image.scale;
...
function z_handler() {
	...
	if("active_box" in image) {
		...
		else if(image.active_box == -2 && image.temp_box.h > 70) {
...
function load_image() {
	...
	xmlhttp.open("GET", "hpu3/get", false)
	xmlhttp.send()
	...
	for(var i = 0; i < len; i++) {
		...
		var h = parseInt(response[4 * i + 3]);
		var w = parseInt(response[4 * i + 4]);
